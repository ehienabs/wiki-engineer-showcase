      ...
      <div className="section-container bg-gray-100">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mt-1 ">
          {projects.map((project, index) => (
            <Card
              key={index}
              className="project-card h-full flex flex-col border-3 hover:border-wikimedia hover:shadow-md transition-all"
            ...
            >
                    ...
                    <img
                      ...
                      className={`object-cover w-full h-full transition-all hover:scale-105 ${
                        project.image.includes("placeholder")
                          ? "w-16 h-16 object-contain m-auto opacity-50"
                          : ""
                      }`}
                    />
                  </AspectRatio>
              ...
              <CardContent className="pt-6 pb-4">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block hover:text-wikimedia transition-colors"
                >
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag) => (
                    <Badge
                      key={tag}
                      variant="outline"
                      className="h-full border-2 hover:border-wikimedia hover:shadow-md text-xs"
                    >
                      {tag}
                    </Badge>
                  ))}
              ...
              <CardFooter className="flex gap-3 pt-0 mt-auto">
                <Button
                  variant="secondary"
                  size="sm"
                  className="border-2 hover:border-wikimedia hover:shadow-md flex-1"
                  asChild
                >
